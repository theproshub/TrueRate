...
- `publish-guard` (PreToolUse, blocking) — a stop-and-confirm tripwire: denies running
  the bulk article importer (`node … import-news-articles`) or a `scripts/publish-*`
  script, so a human confirms every affected article passed `/number-lock` first
  (type 1 gate). It gates only that Bash command, since TrueRate publishes to the
  Supabase `articles` table, not to `src/data/news.ts`.
