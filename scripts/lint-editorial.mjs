...
//
// Errors (exit 1):
//   E1  bad currency prefix — bare "$" or "LD$" before a number (must be US$ or L$) — titles, deks, body
...
const BAD_CURRENCY = /(?<!US)(?<!L)\$\d/;  // flags bare "$" and "LD$" — the Liberian dollar is L$
...
function checkText(where, kind, text) {
  ...
  if (m) errors.push(`E1 bad currency prefix (use US$ / L$) in ${kind} of ${where}: "…${text.slice(Math.max(0, m.index - 20), m.index + 15)}…"`);
