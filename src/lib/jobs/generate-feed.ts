// Daily feed card generation — shared job logic.
//
// Runs from the Vercel route and the droplet runner. No `next/*` imports:
// cache invalidation belongs to the caller.
//
// Unlike the other two jobs this one INSERTs rather than upserts, so a double
// run creates duplicate cards. The 30-minute guard below is the only thing
// preventing that — treat it as load-bearing.
//
// Off Vercel this needs AI_GATEWAY_API_KEY: on-platform the AI Gateway
// authenticates via OIDC, which does not exist on the droplet.
...
export const GENERATE_FEED_REVALIDATE_TAGS = ['feed'] as const;
...
export async function generateFeed(
  ...
  //    The column is `run_at`; querying `created_at` here silently disabled this
  //    guard entirely, because the PostgREST error was never destructured.
  const thirtyMinAgo = new Date(Date.now() - 30 * 60_000).toISOString();
  const { data: recentRun, error: guardErr } = await supabase
    .from('generation_log')
    .select('id')
    .in('status', ['success', 'partial'])
    .gte('run_at', thirtyMinAgo)
    .limit(1);
  // Fail closed. This job inserts, so running blind after a broken guard query
  // is how duplicate cards get created.
  if (guardErr) throw guardErr;
  if (recentRun && recentRun.length > 0) {
    onProgress('skipped: a successful run completed within the last 30 minutes');
    return { ok: true, skipped: true, cards_created: 0, status: 'skipped', detail: {} };
  }
  ...
  try {
    ...
    for (const job of aiJobs) {
      try {
        ...
        onProgress(`${job.type}: ${cards.length} cards`);
      } catch (e) {
        hadError = true;
        runDetail[`${job.type}_error`] = e instanceof Error ? e.message : String(e);
        onProgress(`${job.type} FAILED: ${runDetail[`${job.type}_error`]}`);
      }
    }
    ...
    try {
      ...
      if (tickers.length > 0) {
        ...
        onProgress(`markets: ${tickers.length} tickers`);
      } else {
      ...
      onProgress(`markets FAILED: ${runDetail.markets_error}`);
    }
