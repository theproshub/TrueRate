  ...
  return (
    <div className="mx-auto max-w-container px-4 py-6">
      <div className="animate-pulse space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-6 border-b border-white/[0.08]">
              ...
              {Array.from({ length: 2 }).map((_, i) => (
                <div key={i} className="space-y-3">
                  ...
                  {Array.from({ length: 5 }).map((_, j) => (
                    <div key={j} className="h-4 w-full rounded bg-white/[0.05]" />
                  ))}
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-6 border-b border-white/[0.08]">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="rounded-xl border border-white/[0.07] bg-white/[0.025] p-5 space-y-4">
              <div className="h-4 w-36 rounded bg-white/[0.06]" />
              <div className="h-3 w-full rounded bg-white/[0.04]" />
              {Array.from({ length: 5 }).map((_, j) => (
                <div key={j} className="h-4 w-full rounded bg-white/[0.05]" />
              ))}
              <div className="h-[140px] w-full rounded bg-white/[0.04]" />
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            <div className="h-[280px] w-full rounded-xl bg-white/[0.06]" />
          ...
          <div className="space-y-3">
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="flex gap-3">
                <div className="h-[60px] w-[88px] shrink-0 rounded-lg bg-white/[0.05]" />
