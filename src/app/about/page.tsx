    ...
    <main className="mx-auto max-w-[860px] px-4 py-12">
      <div className="mb-10 text-center">
        <Link href="/" className="inline-flex items-center gap-2 no-underline mb-4">
          <span className="text-[28px] font-bold text-white">TrueRate</span>
          <span className="rounded bg-[#6001d2] px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-widest text-white">Liberia</span>
        </Link>
        <h1 className="mt-4 text-[28px] font-bold text-white">About TrueRate</h1>
        <p className="mt-3 text-[15px] text-gray-500">Liberia&apos;s dedicated financial news, data, and markets platform.</p>
      </div>
      ...
      <div className="space-y-8 text-[15px] leading-relaxed text-[#ccc]">
        ...
        <section>
          ...
          <p className="text-gray-500">TrueRate provides financial information for educational purposes only. Nothing on this site constitutes investment advice. Always conduct your own research or consult a licensed financial advisor before making investment decisions.</p>
        </section>
      </div>
      ...
      <div className="mt-10 flex flex-wrap gap-3 border-t border-white/[0.06] pt-8">
        {[
          ...
          <Link key={l.label} href={l.href} className="rounded-xl border border-white/[0.07] px-4 py-2 text-[13px] text-gray-500 no-underline hover:text-emerald-400 hover:border-emerald-400/50/40 transition-colors">
            {l.label}
          </Link>
        ))}
      </div>
