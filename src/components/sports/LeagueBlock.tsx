    ...
    <section aria-labelledby={`league-${data.key}`} className="mt-12 first:mt-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <Link
          ...
          className="lg:col-span-8 group flex flex-col sm:flex-row gap-4 no-underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-2"
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[10px] font-bold uppercase tracking-wide text-red-700 mb-1.5">{data.featured.category}</p>
            <h3 className="text-[18px] sm:text-[20px] font-bold text-gray-900 leading-snug group-hover:text-gray-700 transition-colors">
              {data.featured.title}
            </h3>
            <p className="mt-2 text-[13px] text-gray-600 leading-relaxed line-clamp-3">{data.featured.dek}</p>
            <p className="mt-2 text-[11px] text-gray-500">
              <span className="font-semibold text-gray-700">{data.featured.source}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
        <ul className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-gray-200 pt-4">
          {data.secondaries.map(s => (
            <li key={s.href}>
              <Link
                href={s.href}
                className="group block no-underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-700 focus-visible:ring-offset-1"
              >
                ...
                <h4 className="text-[13px] font-semibold text-gray-900 leading-snug group-hover:text-gray-700 transition-colors line-clamp-3">
                  {s.title}
                </h4>
        ...
        <div className="lg:col-span-4 lg:border-l lg:border-gray-200 lg:pl-6 border-t lg:border-t-0 border-gray-200 pt-4 lg:pt-0">
