        ...
        {MY_TEAMS.map(t => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-semibold text-white leading-tight truncate">{t.name}</p>
              <p className="text-[10px] uppercase tracking-wide text-gray-500 mt-0.5">{t.league}</p>
              <p className="text-[11px] text-gray-400 mt-1 truncate">{t.next}</p>
            </div>
            {t.last && (
              <span
                className={`shrink-0 px-2 py-1 text-[10px] font-bold tabular-nums ${RESULT_BG[t.last.result]}`}
