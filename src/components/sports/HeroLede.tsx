  ...
  return (
    <section aria-labelledby="sports-hero">
      <Link href={href} className="group block overflow-hidden rounded-xl no-underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent-ink focus-visible:ring-offset-2">
        <HeroVisual category={imageCategory} src={image} className="w-full h-[240px] sm:h-[360px] lg:h-[440px] transition-transform motion-safe:group-hover:scale-[1.02]" />
      </Link>

      <div className="mt-5 max-w-[44rem]">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          {flag && <FlagChip flag={flag} />}
          <span className="text-2xs font-bold uppercase tracking-[0.18em] text-brand-accent-ink">{category}</span>
        </div>
        ...
        <Link href={href} className="group no-underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent-ink focus-visible:ring-offset-2">
          <h1
            id="sports-hero"
            className="text-3xl sm:text-4xl font-bold leading-[1.08] tracking-tight text-gray-900 group-hover:text-gray-700 transition-colors text-balance"
          >
            {title}
          </h1>
        </Link>
        ...
        <p className="mt-4 text-lg leading-relaxed text-gray-600">
          {dateline && <span className="font-bold uppercase tracking-wide text-gray-900">{dateline} — </span>}
          {dek}
        </p>
