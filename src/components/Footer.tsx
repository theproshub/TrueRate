    ...
    <footer className="mt-10 border-t border-white/[0.06] bg-brand-dark">
      ...
      <div className="border-b border-white/[0.05] px-4 py-6">
        <div className="mx-auto max-w-[1320px] flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 id="newsletter-heading" className="text-[12px] font-bold text-white">TrueRate Daily Brief</h3>
            <p className="mt-1 text-[14px] text-gray-400">Business, investing, and technology — delivered daily</p>
          </div>
          <form
            aria-labelledby="newsletter-heading"
            onSubmit={e => e.preventDefault()}
            className="flex flex-col min-[400px]:flex-row w-full max-w-[420px] gap-2.5"
          >
            ...
            <input
              ...
              className="flex-1 rounded-lg bg-white/[0.05] border border-white/[0.08] px-4 py-3 text-[13px] text-white placeholder:text-gray-400 outline-none focus:border-white/60 focus-visible:ring-2 focus-visible:ring-brand-accent transition-colors"
            />
            <button
              type="submit"
              className="shrink-0 rounded-lg bg-white px-6 py-3 text-[13px] font-semibold text-[#0a0a0d] hover:brightness-90 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent focus-visible:ring-offset-2 focus-visible:ring-offset-brand-dark"
            >
              Sign up free
            </button>
          </form>
      ...
      <div className="mx-auto max-w-[1320px] px-4 py-5">
        <div className="flex flex-col gap-5 sm:flex-row sm:gap-16">
            ...
            <ul className="flex items-center gap-3 list-none p-0 m-0">
              <li>
                <a href="https://x.com" target="_blank" rel="noopener noreferrer" aria-label="TrueRate on X"
                  className="flex h-11 w-11 items-center justify-center rounded-full border border-white/20 text-gray-300 hover:text-white hover:border-white/40 transition-colors no-underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-accent">
          ...
          {/* Right: link columns */}
          <nav aria-label="Footer" className="grid grid-cols-2 gap-x-10 gap-y-8 sm:grid-cols-4 flex-1">
              ...
              <div key={title}>
                ...
                <ul className="space-y-3">
                  {links.map(({ label, href }) => (
                    <li key={label}>
                      <Link href={href} className="text-[13px] text-gray-300 hover:text-white no-underline transition-colors focus-visible:outline-none focus-visible:underline">
                        {label}
                      </Link>
                    </li>
                  ))}
