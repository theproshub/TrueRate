  ...
  if (compact) {
    return (
      <article className="py-4 first:pt-0">
        <Link href={href} className={`group flex gap-4 no-underline ${focusRing}`}>
          ...
          <div className="min-w-0 flex-1 flex flex-col justify-start">
            <h3 className="text-[18px] font-bold leading-[1.35] text-[#0A0A0A] group-hover:underline decoration-1 underline-offset-2 line-clamp-2">
              {title}
            </h3>
            <SourceLabel source={source} time={time} className="mt-2" />
          </div>
        </Link>
      </article>
    );
