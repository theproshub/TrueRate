...
export default function StoryCard({ category, categorySlug, title, source, time, href, image, horizontal = false }: Props) {
  if (horizontal) {
    return (
      <article>
        <Link href={href} className={`group flex gap-3.5 no-underline ${focusRing}`}>
          <div className="shrink-0 overflow-hidden rounded-lg">
            <NewsThumbnail
              category={categorySlug ?? category}
              src={image ?? undefined}
              className="h-[88px] w-[88px] group-hover:scale-[1.03] transition-transform duration-300"
            />
          </div>
          <div className="min-w-0 flex-1">
            ...
            <h3 className="mt-0.5 text-[14px] font-bold leading-[1.3] text-[#0A0A0A] group-hover:underline decoration-1 underline-offset-2 line-clamp-3">
              {title}
            </h3>
